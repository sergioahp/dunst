...
=item B<action_history_timeout> (default: 0)

How long actions stay valid after the notification is closed. Supports the same
time suffixes as other timeout values. Set to 0 to drop actions immediately
...
setting. Actions are invalidated once the notification is closed unless
B<action_history_timeout> is set to a value greater than 0, so with the default
configuration you cannot execute an action when you bring back a notification
from history.
